...
def main():
	...
	OutputFile = "MasterRNASeq.txt"

	RNA_list = ListMaker(RNA_Names) #a list of genes in the same order as the excel file (to make formatting easier)
	IDlist, IDDict, NameDict = IDDict_maker(Name_Map) #makes name>ID/ID>name dictionaires

	basemean_Dict, log2_Dict, lfcSE_Dict, stat_Dict, pval_Dict, padj_Dict = excel_reader(RNA_Excel, RNA_list)

	FilePrinter(basemean_Dict, log2_Dict, lfcSE_Dict, stat_Dict, pval_Dict, padj_Dict, OutputFile, RNA_list, IDDict)
	return
...
def FilePrinter(basemean_Dict, log2_Dict, lfcSE_Dict, stat_Dict, pval_Dict, padj_Dict, OutputFile, RNA_list, IDDict):
	f = open(OutputFile, "w")
	print("Opening ", OutputFile)

	for gene in RNA_list:
		common_name = IDDict[gene]
		basemean = basemean_Dict[gene]
		log2 = log2_Dict[gene]
		lfcSE = lfcSE_Dict[gene]
		stat = stat_Dict[gene]
		pval = pval_Dict[gene]
		padj = padj_Dict[gene]
		f.write(gene + '\t' + common_name + '\t' + basemean + '\t' + log2 + '\t' + lfcSE + '\t'+ stat + '\t' + pval + '\t' + padj + '\n')
